...
function loadHystrixCommand(window) {

	var data1 = (data) => `<div class="counters">
		
<div class="cell line">
	<a href="javascript://" title="Error Percentage [Timed-out + Threadpool Rejected + Failure / Total]" class="tooltip errorPercentage"><span class="value">${data.errorPercentage}</span> %</a>
</div>

<div class="cell borderRight">
	<a href="javascript://" title="Timed-out Request Count" class="line tooltip timeout">${data.addCommas(data.rollingCountTimeout)}</a>
	` + `
	${data.propertyValue_executionIsolationStrategy == 'THREAD' ?
		`<a href="javascript://" title="Threadpool Rejected Request Count" class="line tooltip rejected">${data.addCommas(data.rollingCountThreadPoolRejected)}</a>`
		: ""
	}
	${data.propertyValue_executionIsolationStrategy == 'SEMAPHORE' ?
		`<a href="javascript://" title="Semaphore Rejected Request Count" class="line tooltip rejected">${data.addCommas(data.rollingCountSemaphoreRejected)}</a>`
		: ""
	}
	<a href="javascript://" title="Failure Request Count" class="line tooltip failure">${data.addCommas(data.rollingCountFailure)}</a> 
</div>
<div class="cell borderRight">
	<a href="javascript://" title="Successful Request Count" class="line tooltip success">${data.addCommas(data.rollingCountSuccess)}</a>
	<a href="javascript://" title="Short-circuited Request Count" class="line tooltip shortCircuited">${data.addCommas(data.rollingCountShortCircuited)}</a>
	<a href="javascript://" title="Bad Request Count" class="line tooltip badRequest">${data.addCommas(data.rollingCountBadRequests)}</a>
	<br>
</div>
</div>

<div class="rate">
<a href="javascript://" title="Total Request Rate per Second per Reporting Host" class="tooltip rate"><span class="smaller">Host: </span><span class="ratePerSecondPerHost">${data.addCommas(data.roundNumber(data.ratePerSecondPerHost))}</span>/s</a>
</div>
<div class="rate">	
<a href="javascript://" title="Total Request Rate per Second for Cluster" class="tooltip rate"><span class="smaller">Cluster: </span><span class="ratePerSecond">${data.addCommas(data.roundNumber(data.ratePerSecond))}</span>/s</a>
</div>

<div class="circuitStatus">
 ${data.propertyValue_circuitBreakerForceClosed ?
...
<div class="tableRow">
	${typeof data.reportingHosts != 'undefined' ?
	`<div class="cell header">Hosts</div>
	<div class="cell data">${data.reportingHosts}</div>`
	:
	`<div class="cell header">Host</div>
	<div class="cell data">Single</div>`
 	}
	<div class="cell header">90th</div>
	<div class="cell data latency90"><span class="value">${data.getInstanceAverage(data.latencyExecute['90'], data.reportingHosts, false)}</span>ms</div>
</div>
<div class="tableRow">
	<div class="cell header">Median</div>
	<div class="cell data latencyMedian"><span class="value">${data.getInstanceAverage(data.latencyExecute['50'], data.reportingHosts, false)}</span>ms</div>
	<div class="cell header">99th</div>
	<div class="cell data latency99"><span class="value">${data.getInstanceAverage(data.latencyExecute['99'], data.reportingHosts, false)}</span>ms</div>
</div>
<div class="tableRow">
	<div class="cell header">Mean</div>
	<div class="cell data latencyMean"><span class="value">${data.latencyExecute_mean}</span>ms</div>
	<div class="cell header">99.5th</div>
	<div class="cell data latency995"><span class="value">${data.getInstanceAverage(data.latencyExecute['99.5'], data.reportingHosts, false)}</span>ms</div>
</div>`

var data2 = (data) => `<div class="monitor" id="CIRCUIT_${data.name + '_' + data.index}" style="position:relative;">

<div id="chart_CIRCUIT_${data.name + '_' + data.index}" class="chart" style="position:absolute;top:0px;left:0; float:left; width:100%; height:100%;"></div>
...
<div id="graph_CIRCUIT_${data.name + '_' + data.index}" class="graph" style="position:absolute;top:25px;left:0; float:left; width:140px; height:62px;"></div>

<script>
	var y = 200;
	/* escape with two backslashes */
	var vis = d3.select("#chart_CIRCUIT_${data.name.replace(/([ !"#$%&'()*+,./:;<=>?@[\]^\`{|}~])/g,'\\\\$1') + '_' + data.index}").append("svg:svg").attr("width", "100%").attr("height", "100%");
	...
	/* escape with two backslashes */
	var graph = d3.select("#graph_CIRCUIT_${data.name.replace(/([ !"#$%&'()*+,./:;<=>?@[\]^\`{|}~])/g,'\\\\$1') + '_' + data.index}").append("svg:svg").attr("width", "100%").attr("height", "100%");
</script>
</div>
`

	// cache the templates we use on this page as global variables (asynchronously)
		hystrixTemplateCircuit = data1;
		hystrixTemplateCircuitContainer = data2;
	...
	window.HystrixCommandMonitor = function(index, containerId, args) {
		...
		/* private */ function displayCircuit(data) {
			...
			if(!$('#CIRCUIT_' + data.escapedName).length) {
				...
				var html = hystrixTemplateCircuitContainer(data);
			...
			$('#CIRCUIT_' + data.escapedName + ' div.monitor_data').html(hystrixTemplateCircuit(data));
